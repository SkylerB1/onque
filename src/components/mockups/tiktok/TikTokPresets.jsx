...
function TikTokPresets({
  ...
  const handleChange = (identifier, value) => {
    setAdditionalPresets((prev) => ({
      ...
      [platform]: {
        ...prev[platform],
        [identifier]: value || "PUBLIC_TO_EVERYONE",  // Default to "PUBLIC_TO_EVERYONE" if value is empty
      },
    }));
  ...
  const getCreatorInfo = async () => {
    try {
      ...
      setAdditionalPresets((prev) => ({
        ...
        [platform]: {
          ...prev[platform],
          privacyLevel: prev[platform]?.privacyLevel || "PUBLIC_TO_EVERYONE",  // Set default if not already set
          commentDisabled: comment_disabled,
          duetDisabled: duet_disabled,
          stitchDisabled: stitch_disabled,
          maxVideoPostDuration: max_video_post_duration_sec,
          ...
          <>
            <div className="flex flex-row">
              <SelectInput
                ...
                label={"Who can view your post?"}
                options={memoizedPrivacyOptions}
                value={additionalPresets?.privacyLevel || "PUBLIC_TO_EVERYONE"}
                onChange={handleChange}
              />
            </div>
            <div className="flex flex-row justify-evenly my-4">
