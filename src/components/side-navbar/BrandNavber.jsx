    ...
    return (
        <div>
            <div className="ml-2">
                ...
                <ListItem className=" w-[260px] mt-8 hover:bg-[#fde8ef] hover:text-[#ec407a]" onClick={() => setOpen(true)} >
                    <ListItemPrefix>
                        <MdAutoDelete className="h-5 w-5" />
                    </ListItemPrefix>
                    <p className="w-full text-base text-[#646BFE]">
                        Delete client
                    </p>
                </ListItem>
            </div>
            <CustomModal open={opens} Close={() => setOpen(false)} title={`Are you sure that you want to delete ${user?.firstName + " " + user?.lastName}?`} handleDelete={handleDeleteClient} id={brandId} />
        </div>
    )
}
